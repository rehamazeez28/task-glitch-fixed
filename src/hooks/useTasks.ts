...
export function useTasks(): UseTasksState {
  ...
  const resetDeletedTask = useCallback(() => {
    setLastDeleted(null);
  }, []);

  return { tasks, loading, error, derivedSorted, metrics, lastDeleted, addTask, updateTask, deleteTask, undoDelete, resetDeletedTask };
}
